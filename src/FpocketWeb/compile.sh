...
# Fix version
export FPOCKETWEB_VERSION=`cat ../Version.ts  | grep VERSION | awk '{print $5}' | sed 's/"//g' | sed "s/;//g"`
grep -l "XXXXXXXXXXXXX.X" dist/* | awk '{print "cat " $1 " | sed \"s/XXXXXXXXXXXXX.X/TMPSTR/g\" > t; mv t " $1}' | sed "s/TMPSTR/${FPOCKETWEB_VERSION}/g" | bash

cd dist
echo "/**
 * FPocketWeb Copyright 2020 Jacob Durrant
 *
