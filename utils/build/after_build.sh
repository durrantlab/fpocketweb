...
cd dist
ls vendors*js runtime*js styles*js FpocketWeb/*.js | awk '{print "echo Compiling " $1 ";node ../node_modules/google-closure-compiler/cli.js " $1 " > t; mv t " $1}' | bash
cd -
 ...
 */" > t
ls app.*js FpocketWeb/*.js | awk '{print "cat t > " $1 ".tmp; cat " $1 " >> " $1 ".tmp; mv " $1 ".tmp " $1}' | bash
rm t
cd -

# Consider app.js. Used regex to replace >\\n\s+< with ><
# This is to remove the extra whitespace between the tags.
cd dist
# ls app.*js | awk '{print "cat " $1 " | sed \"s|>\\\\\\n *<|> <|g\" > " $1 ".tmp; mv " $1 ".tmp " $1}' | bash
ls app.*js | awk '{print "cat " $1 " | sed \"s|\\\\n *| |g\" > " $1 ".tmp; mv " $1 ".tmp " $1}' | bash
cd -

# Also create a ZIP file of the dist directory, for convenient distribution.
mv dist fpocketweb
zip -r fpocketweb.zip fpocketweb
mv fpocketweb dist
